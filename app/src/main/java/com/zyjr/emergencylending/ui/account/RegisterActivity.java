        ...
        subscription = RxView.clicks(btnRegister).throttleFirst(2, TimeUnit.SECONDS).subscribe(new Action1<Void>() {
            ...
            public void call(Void aVoid) {
                ...
                if (TextUtils.isEmpty(phone)) {
                    ...
                    mPresenter.register(NetConstantValues.REGISTER, phone, SPUtils.getWyString(mContext, Config.KEY_CLIENT_ID), sms, pwd, inviteCode, Constants.getPlatform(1), NetworkUtils.getIp(mContext), Constants.getDeviceCode(), WYUtils.getDeviceFingerprinting(mContext));
                }
            }
        });
