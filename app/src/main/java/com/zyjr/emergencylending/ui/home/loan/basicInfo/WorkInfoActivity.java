...
public class WorkInfoActivity extends BaseActivity {
    ...
    @OnClick({R.id.ll_job_unit_industry, R.id.ll_job_address, R.id.ll_unit_job, R.id.ll_income})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.ll_job_address:  // 工作地址
                AreaSelectPop popWorkAddressSelect = new AreaSelectPop(this, CommonUtils.mProvinceDatas, CommonUtils.mCitisDatasMap, CommonUtils.mDistrictDatasMap);
                popWorkAddressSelect.setOnCityPopupWindow(new AreaSelectPop.OnCityPopupWindow() {
                    @Override
                ...
                if (!TextUtils.isEmpty(UserInfoManager.getInstance().getLocation().getmCurrentCity())) {
                    popWorkAddressSelect.setWheel();
                }
                popWorkAddressSelect.showAtLocation(getRootView(), Gravity.BOTTOM, 0, 0);
                break;
